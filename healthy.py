...
def notice(key, title, msg):
    ...
    if text['data'] == None:
        ...
        return
    data = text['data']
    print("[SCTPUSH]Info: [用户ID]:%s , [标题]:%s , [内容]:%s , [建立时间]:%s" % (
    data['uid'], data['title'], data['desp'], data['created_at']))

if __name__ == "__main__":
    push = os.getenv('push', None)
    ...
    print("[Process]Info: 打卡信息: %s" % (json.dumps(body, indent=4, ensure_ascii=False)))
    url = 'http://xggl.hnqczy.com/content/student/temp/zzdk?_t_s_=' + timeStamp()
    res = json.loads(request.post(url, headers=header, data=body).text)
    if res['result'] == False:
        print("[Process]Error: %s" % res['errorInfoList'][0]['message'])
        print("[Process]Info: 打卡失败，推送失败信息...")
        notice(push, "打卡失败", "操作用户：%s\r\n\r\n操作日志：%s\r\n\r\n操作时间：%s(UTC)" % (username, (res['errorInfoList'][0]['message']), str(datetime.now())))
    else:
